...
RUN bash /miniconda.sh -b -f -p /miniconda/ && rm /miniconda.sh && export PATH=$PATH:/miniconda/bin/ && \
	...
	conda install -y -c bioconda mtbseq && cd / && mkdir /miniconda/mtbdependencies/ && \ 
...
RUN gatk3-register /miniconda/mtbdependencies/GenomeAnalysisTK-3.8-1-0-gf15c1c3ef.tar.bz2 && \
